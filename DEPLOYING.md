...
```
pack build python-azure --path . --builder ghcr.io/vmware-tanzu/function-buildpacks-for-knative/functions-builder:0.3.1 --env BP_FUNCTION=func.main
```

Where `python-azure` is the name of your runnable function image.

Then run via Docker:

```
docker run -it --rm -p 8080:8080 python-azure
```
...
```
tanzu apps workload create python-azure -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/python-azure-source \
  --type web
```
...
```
tanzu apps workload get python-azure
```
